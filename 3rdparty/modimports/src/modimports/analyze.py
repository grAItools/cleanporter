...
def analyze_record(rec: FileRecord, resolver: Resolver, config: Config) -> list[Finding]:
    ...
    for unit in iter_units(rec.tree, rec.base_pkg):
        ...
        if unit.star:
            findings.append(
                Finding(rec.path, line, col, unit.parent or "?", "*",
                        Status.UNFIXABLE, "wildcard import cannot be rewritten to a module import")
            )
            continue
        if unit.parent is None:
            findings.append(
                Finding(rec.path, line, col, "?", unit.name, Status.UNKNOWN,
        ...
        if verdict is None:
            findings.append(
                Finding(rec.path, line, col, unit.parent, unit.name, Status.UNKNOWN,
    ...
    for f in files:
        ...
        try:
            ...
            errors.append(Finding(f, exc.raw_line, exc.raw_column, "?", "?",
                                  Status.UNKNOWN, f"parse error: {exc.message}"))
            continue
